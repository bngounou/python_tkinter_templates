...
class ScrollCanvas(Tk) :
    def __init__(self):
        ...
        myscroll_x.grid(row=0, column=1, sticky=EW)
        myscroll_y = Scrollbar(self, command=mycanvas.yview)
        myscroll_y.grid(row=1, column=0, sticky=NS)

        mycanvas.configure(scrollregion=mycanvas.bbox('all')) # will return the rectangular coordinates fitting
        # the whole canvas content. Here the position of 2 corners of a
